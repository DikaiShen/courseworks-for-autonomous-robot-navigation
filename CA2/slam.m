...
for i = 1:size(output_temp,1)
	...
	for j = 1:particles_count
        ...
        thetaNoises = motion_estimate(j).theta*(1+theta_noise_proportion*randn(1)) + theta_noise_add*randn(1);
        transNoisesX = motion_estimate(j).x*(1+translation_noise_proportion*randn(1)) + translation_noise_add*randn(1);
        transNoisesY = motion_estimate(j).y*(1+translation_noise_proportion*randn(1)) + translation_noise_add*randn(1);

        particles(j).theta = particles(j).theta + thetaNoises;
        % Apply translation to all particles with (additive and proportional) noises 
        particles(j).x = particles(j).x + cos(particles(j).theta)*transNoisesX - sin(particles(j).theta)*transNoisesY;
        particles(j).y = particles(j).y + sin(particles(j).theta)*transNoisesX + cos(particles(j).theta)*transNoisesY;
