	...
	for i = 1:size(lines, 2)-1
		if (lines(i).p2.x == lines(i+1).p1.x && lines(i).p2.y == lines(i+1).p1.y)
			...
			if (lines(i).length > length_min && lines(i+1).length > length_min)
                ...
                Va = [0,0];
                Vb = [0,0];
                Va(1) = lines(i).p2.x - lines(i).p1.x;
                Va(2) = lines(i).p2.y - lines(i).p1.y;

                Vb(1) = lines(i+1).p2.x - lines(i+1).p1.x;
                Vb(2) = lines(i+1).p2.y - lines(i+1).p1.y;

                angle = acos((Va*Vb')/(norm(Va)*norm(Vb)));
