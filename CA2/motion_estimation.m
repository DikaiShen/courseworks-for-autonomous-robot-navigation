...
for i = 1:size(output_temp,1)-1
	...
	if (assoc_count >= 2)
        % Missing codes start here ...
   		...
   		% the two frames 
        Pmean = [mean(P(:,1)),mean(P(:,2))];
        Qmean = [mean(Q(:,1)),mean(Q(:,2))];
        Pi = zeros(size(P,1),size(P,2));
        Qi = zeros(size(Q,1),size(Q,2));
        for j = 1:size(P,1)
            Pi(j,:) = P(j,:) - Pmean;
        end
        for j = 1:size(Q,1)
        ...
        [U,S,V] = svd(Pi*Qi');
        d = sign(det(V*U'));
        R = V*[1 0;0 d]*U'; % rotation calculation
        Pmean = Pmean';
        Qmean = Qmean';
        translation = R*Pmean - Qmean;
        theta = -atan2(R(2), R(1));
